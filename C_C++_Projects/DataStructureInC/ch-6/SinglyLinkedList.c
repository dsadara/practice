...
int main(void)
{
    ListNode *head = NULL;  // 공백 리스트의 생성
    for (int i = 0 ; i < 5 ; i++)
    {
        head = insert_first(head, i);   // insert_first()에서 반환된 헤드 포인터를 head 에 대입한다 
        // -> 포인터가 가리키는 것의 변수는 바꿀 수 있지만 포인터 자체는 복사본이므로 바꿀 수 없다 그래서 반환값을 head 에 저장한다
        print_list(head);  
    }
    printf("index = 4 의 데이터를 찾아서 반환 : %d\n", get_entry(head, 4));
    printf("노드의 갯수를 출력 : %d\n", get_length(head));
    for (int i = 0 ; i < 5 ; i++)
    {
